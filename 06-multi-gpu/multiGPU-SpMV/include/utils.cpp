...
template<typename IdxType, typename ValType>
void* my_mtx_parser(int argc, char *argv[], const char* str_outtype)
{
    MM_typecode matcode;
    FILE *f;
    int i;
    ...
    struct DENSE<ValType>          *dense = NULL;
    struct COO<IdxType, ValType>   *coo   = NULL;
    struct CSR<IdxType, ValType>   *csr   = NULL;

    if (strcmp(str_outtype, "dense") == 0) {
        dense = my_mtx_to_dense<ValType>(f, &matcode);
    } else if (strcmp(str_outtype, "coo") == 0) {
        coo = my_mtx_to_coo<IdxType, ValType>(f, &matcode);
    } else if (strcmp(str_outtype, "csr") == 0) {
        csr = my_mtx_to_csr<IdxType, ValType>(f, &matcode);
    } else {
...
typedef struct DENSE<VALTYPE> myDense;
typedef struct COO<IDXTYPE, VALTYPE> myCOO;
typedef struct CSR<IDXTYPE, VALTYPE> myCSR;
